...
public class ProductController {
  ...
  public ResponseEntity<List<ProductDTO>> getProductsByCategory(
      @Parameter(description = "ID da categoria para filtrar produtos", required = true)
          @PathVariable
          Long categoryId) {
  ...
  @Operation(
      summary = "Obter produtos por marca",
      description = "Retorna todos os produtos pertencentes a uma marca específica",
      tags = {"Produtos"})
  @ApiResponses(
      value = {
        @ApiResponse(
  ...
  public ResponseEntity<List<ProductDTO>> getProductsByBrand(
      @Parameter(description = "ID da marca para filtrar produtos", required = true) @PathVariable
          Long brandId) {
  ...
  @ApiResponses(
      value = {
        ...
        @ApiResponse(
            responseCode = "404",
            description = "Categoria ou marca referenciada não encontrada",
            content =
                @Content(
                    mediaType = "application/json",
                    schema = @Schema(implementation = ErrorResponse.class))),
        @ApiResponse(
            responseCode = "409",
            description = "Produto com o mesmo SKU já existe",
            content =
                @Content(
                    mediaType = "application/json",
                    schema = @Schema(implementation = ErrorResponse.class))),
  ...
  public ResponseEntity<ProductDTO> createProduct(
      @Parameter(description = "Detalhes do produto para criação", required = true)
          @Valid
          @RequestBody
  ...
  @ApiResponses(
      value = {
        ...
        @ApiResponse(
            responseCode = "404",
            description = "Produto não encontrado ou categoria/marca referenciada não encontrada",
            content =
                @Content(
                    mediaType = "application/json",
                    schema = @Schema(implementation = ErrorResponse.class))),
        @ApiResponse(
            responseCode = "409",
            description = "SKU do produto conflita com um produto existente",
            content =
                @Content(
                    mediaType = "application/json",
                    schema = @Schema(implementation = ErrorResponse.class))),
  ...
  public ResponseEntity<ProductDTO> updateProduct(
      @Parameter(description = "ID do produto a ser atualizado", required = true) @PathVariable
          Long id,
      @Parameter(description = "Informações atualizadas do produto", required = true)
          @Valid
          @RequestBody
          ProductDTO productDTO) {
  ...
  @ApiResponses(
      value = {
        ...
        @ApiResponse(
            responseCode = "409",
            description = "Não é possível excluir o produto porque ele está referenciado em vendas",
            content =
                @Content(
                    mediaType = "application/json",
                    schema = @Schema(implementation = ErrorResponse.class))),
  ...
  public ResponseEntity<SuccessResponse> deleteProduct(
      @Parameter(description = "ID do produto a ser excluído", required = true) @PathVariable
          Long id) {
